...
const StudentProfileCreation = () => {
  ...
  const currentQuestionKey = questionKeys[stepData.currentQuestion];
  if (currentQuestionKey === 'sector_interests') {
    toast.info('Voice input is not available for sector selection. Please use the dropdown.');
    return;
  }
...
const stopVoiceRecognition = () => {
  ...
  // Handle review and auto-submit - Updated for sectors
  useEffect(() => {
    if (currentStep === 'review' && showReview) {
      const sectorNames = selectedSectors.map(s => s.name).join(', ');
      const reviewText = `You are ${profileData.name}, ${profileData.education}, skilled in ${profileData.skills.join(', ')}, interested in ${sectorNames}, looking for opportunities in ${profileData.preferred_locations.join(', ')}. Auto-submitting in ${autoSubmitCountdown} seconds.`;
  ...
  }, [currentStep, showReview, autoSubmitCountdown, profileData, selectedSectors]);
  ...
  return (
    <Box sx={{ mt: 2 }}>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Select your areas of interest from the official PM Internship Scheme sectors
      </Typography>
      
      {micPersistentMode && (
        <Alert severity="info" sx={{ mb: 2 }}>
          Voice input is paused for sector selection. It will resume for the next question.
        </Alert>
      )}
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField
              {...params}
              variant="outlined"
              label="Select Sector Interests"
              placeholder="Choose sectors that interest you..."
              helperText="Voice input will resume for the next question"
            />
          )}
    ...
    return (
      <Box sx={{ mt: 2 }}>
        ...
        {isListening && (
          <Alert severity="info" sx={{ mb: 2 }}>
            <Typography variant="body2">
              🎤 Listening... Speak phrases clearly and pause between different items. 
              Example: "Software development" (pause) "Machine learning" (pause) "Web design"
            </Typography>
            {voiceBuffer && (
              <Typography variant="body2" sx={{ mt: 1, fontStyle: 'italic' }}>
                Detected: "{voiceBuffer}"
              </Typography>
            )}
          </Alert>
        )}
        ...
        <CardContent sx={{ py: 4 }}>
          ...
          {isSectorQuestion && (
            <Typography variant="body2" color="text.secondary" sx={{ mt: 1, mb: 2 }}>
              Select your areas of interest from the official PM Internship Scheme sectors. 
              Voice input is not available for this step - please use the dropdown selection.
            </Typography>
          )}
          ...
          {isTagQuestion && !isSectorQuestion && (
            <Typography variant="body2" color="text.secondary" sx={{ mt: 1, mb: 2 }}>
              ...
              Note: For technical terms like "C", "C++", "C#", etc - you may need to type them manually if voice recognition doesn't work well.
            </Typography>
          )}
        ...
        <CardContent sx={{ py: 4 }}>
          ...
          <Grid container spacing={3} sx={{ mt: 2 }}>
            <Grid item xs={12} sm={6}>
              <Paper sx={{ p: 2 }}>
                <Typography variant="h6" color="primary">{t('name')}</Typography>
                <Typography variant="body1">{profileData.name}</Typography>
              </Paper>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Paper sx={{ p: 2 }}>
                <Typography variant="h6" color="primary">{t('education')}</Typography>
                <Typography variant="body1">{profileData.education}</Typography>
              </Paper>
            </Grid>
            <Grid item xs={12}>
              <Paper sx={{ p: 2 }}>
                <Typography variant="h6" color="primary" gutterBottom>{t('skills')}</Typography>
                <Box sx={{ 
            ...
            <Grid item xs={12}>
              <Paper sx={{ p: 2 }}>
                <Typography variant="h6" color="primary" gutterBottom>Sector Interests</Typography>
                <Box sx={{ 
                  border: '1px solid #e0e0e0', 
                  borderRadius: 1, 
                  p: 2, 
            ...
            <Grid item xs={12}>
              <Paper sx={{ p: 2 }}>
                <Typography variant="h6" color="primary" gutterBottom>{t('preferredLocations')}</Typography>
                <Box sx={{ 
                  border: '1px solid #e0e0e0', 
                  borderRadius: 1, 
                  p: 2, 
          ...
          {!showReview ? (
            ...
            <Box textAlign="center" sx={{ mt: 4 }}>
              <Alert severity="info" sx={{ mb: 2 }}>
                {t('autoSubmitting', { count: autoSubmitCountdown })}
              </Alert>
              <Button
                variant="outlined"
                color="error"
        ...
        <CardContent sx={{ py: 4 }}>
          ...
          <Box textAlign="center" sx={{ mt: 4 }}>
            <Button
              ...
              startIcon={<VolumeUpIcon />}
              onClick={() => {
                const text = recommendations.map((int, i) => 
                  `${i + 1}. ${int.title} at ${int.company}, ${int.matchScore || 85}% match`
                ).join('. ');
                speakText(`Here are your recommendations: ${text}`);
              }}
              sx={{ mr: 2 }}
            >
              {t('readAloud')}
