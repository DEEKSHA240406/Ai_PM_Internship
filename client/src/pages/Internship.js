...
function applySortFilter(array, comparator, query, filters) {
    ...
    if (filters) {
        filteredArray = filteredArray.filter(internship => {
            ...
            // Sector filtering
            const sectorsMatch = !filters.sectors || internship.sectors?.some(sector => {
                const sectorName = getSectorName(sector);
            ...
            return titleMatch && companyMatch && locationMatch && skillsMatch && 
                   sectorsMatch && statusMatch && remoteMatch && durationMatch && 
                   jobIdMatch && stipendMatch;
        });
    }
...
export default function Internship() {
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = filteredInternships.length > 0
    ...
    const getVisibleSelectedCount = () => {
        return selected.filter((id) =>
            filteredInternships.some((internship) => internship._id === id)
        ).length;
    };
    ...
    const handleDeleteInternship = async (internshipId) => {
        if (window.confirm("Are you sure you want to delete this internship?")) {
            try {
                await axios.delete(`http://localhost:7070/api/internships/admin/internships/${internshipId}`, {
    ...
    const handleExportCSV = () => {
        ...
        const processedData = filteredInternships.map(internship => {
            ...
            if (visibleColumns.includes('stipend')) {
                internshipData.stipend = internship.stipend?.amount ? `₹${internship.stipend.amount.toLocaleString()}` : 'Not specified';
            }
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {visibleColumns.includes('remote_ok') && (
                                                    <TableCell align="left">
                                                        <Label color={row.remote_ok ? 'success' : 'default'}>
                                                            {row.remote_ok ? 'Yes' : 'No'}
                                                        </Label>
                                                    </TableCell>
                                                )}
