...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get(`https://ai-pm-internship-backend.onrender.com/api/auth/profile/${id}`);
  ...
  const requestOtp = async () => {
    try {
      await axios.post('https://ai-pm-internship-backend.onrender.com/api/auth/request-otp', { email }); // Send OTP to user's email
  ...
  const handleChangePassword = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://ai-pm-internship-backend.onrender.com/api/auth/change-password', {
        email: user.email,
        currentPassword,
        newPassword,
        otp,
